...
const double wheelCircumference = wheelDiameter * π;
const double ticksPerRev = 360;
...
void travel(double xcor, double ycor) {
  ...
  double angleDiff = atan2(ydiff,xdiff) * 180 / π - heading - quart;
  turn(angleDiff);
  fwdrev(distance);
}
